...
//%h %{X-Forwarded-For}i %l %u %t \"%r\" %>s %b \"%{Referer}i\" \"%{User-Agent}i\" %D %{SPF_DC_SID}C+%{SPF_DC_ID}i
// Refer to http://httpd.apache.org/docs/2.0/mod/mod_log_config.html#formats
@ParserPattern(value = "SPF_WEB_ACCESS_LOG", parameters = { @Parameter(value = "{REMOTE_HOST} {X_FORWARDED_FOR} - {:REMOTE_USER} [{REV_DATE_TIME}] \"{REQUEST_METHOD} {REQUEST_LINE} HTTP/{HTTP_VERSION}\" {HTTP_STATUS} {RESPONSE_SIZE} \"{REFERER}\" \"{USER_AGENT}\" {TIME_TAKEN} {SPF_DC_SID}+{SPF_DC_ID}") }, parser = DelimsTextParser.class)
public class SPFWebAccessLog implements AcBeanParseHook {
	...
	@ParserPattern(value = "X_FORWARDED_FOR", parameters = {
			@Parameter(", "),
	...
	public void onReady(AcContentLine contentLine, AcTextParserContext context) {
		...
		remoteAddress = forwardedFor[forwardedFor.length-1];
		if (normalize(remoteAddress).length()==0) {
			// no x-forwarded-for
